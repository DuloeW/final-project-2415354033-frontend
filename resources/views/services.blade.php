...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>{{ config('app.name', 'ERP Dashboard') }} - Services</title>
        @fonts
    ...
    <body class="min-h-screen bg-slate-50 text-slate-800 antialiased">
                    ...
                    <div class="mx-auto flex max-w-7xl flex-col gap-6">
                        <header class="rounded-2xl border border-white/80 bg-white/80 px-5 py-4 shadow-sm backdrop-blur">
                            <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                                <div>
                                    <p class="text-sm font-medium text-slate-500">ERP / Services</p>
                                    <h1 class="mt-1 text-2xl font-semibold tracking-tight text-slate-900">Services</h1>
                                </div>

                                <div class="flex items-center gap-3">
                                    <button type="button" data-open-dialog="service-modal" class="inline-flex items-center gap-2 rounded-xl bg-slate-800 px-4 py-2 text-sm font-medium text-white shadow-sm transition hover:bg-slate-700">
                        ...
                        <section class="overflow-visible rounded-2xl border border-slate-200/70 bg-white/90 shadow-sm backdrop-blur">
                            <div class="border-b border-slate-200/70 px-5 py-4">
                                <div class="flex items-center justify-between gap-4">
                                    <div>
                                        <h2 class="text-lg font-semibold text-slate-900">Service List</h2>
                                        <p class="mt-1 text-sm text-slate-500">Daftar layanan yang tampil mengikuti layout referensi.</p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="overflow-x-auto overflow-y-visible">
                                <table class="min-w-full divide-y divide-slate-200/80 text-left text-sm">
                                    <thead class="bg-slate-50/80 text-xs uppercase tracking-wide text-slate-500">
                                        <tr>
                                            <th class="px-5 py-4 font-medium">Service Name</th>
                                    ...
                                    <tbody class="divide-y divide-slate-200/70">
                                        @forelse ($services as $index => $service)
                                            @php
                                                $statusLabel = data_get($service, 'status') ? 'Active' : 'Inactive';
                                                $statusClass = $statusLabel === 'Active'
                                                    ? 'bg-emerald-100 text-emerald-700 ring-1 ring-emerald-200'
                                                    : 'bg-rose-100 text-rose-700 ring-1 ring-rose-200';
                                            @endphp
                                            <tr class="hover:bg-slate-50/70">
                                                <td class="whitespace-nowrap px-5 py-4 font-medium text-slate-900">{{ data_get($service, 'name', '-') }}</td>
                                                <td class="whitespace-nowrap px-5 py-4 text-slate-700">{{ data_get($service, 'price', '-') }}</td>
                                                <td class="whitespace-nowrap px-5 py-4">
                                                    <span class="inline-flex rounded-full px-2.5 py-1 text-xs font-semibold {{ $statusClass }}">{{ $statusLabel }}</span>
                                                </td>
                                                <td class="whitespace-nowrap px-5 py-4 text-right">
                                                    <button type="button" data-action-menu-trigger="service-action-{{ $index }}" class="inline-flex rounded-lg px-2 py-1 text-slate-500 transition hover:bg-slate-100 hover:text-slate-700">
                                        ...
                                        @empty
                                            <tr>
                                                <td colspan="4" class="px-5 py-10 text-center text-sm text-slate-500">No services found.</td>
                                            </tr>
                                        @endforelse
                                    </tbody>
                                </table>
        ...
        @foreach ($services as $index => $service)
            <div id="service-action-{{ $index }}" data-action-menu-panel data-open="false" class="fixed z-[100] hidden w-72 rounded-2xl border border-slate-200 bg-white p-3 shadow-[0_18px_40px_rgba(15,23,42,0.12)]" style="top: 0; left: 0;">
                <div class="space-y-1">
                    ...
                    <form method="POST" action="{{ route('services.destroy', data_get($service, 'id')) }}">
                        @csrf
                        @method('DELETE')
        ...
        <x-service-modal :action="$editingService ? route('services.update', data_get($editingService, 'id')) : route('services.store')" :service="$editingService" />
    </body>
</html>